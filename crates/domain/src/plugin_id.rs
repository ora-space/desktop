...
///
/// This type enforces the structural shape only: both segments are non-empty and limited to
/// lowercase ASCII letters, digits, `-`, and `.`. The full slug grammar (segment count, hyphen
/// placement, length budgets) is owned by manifest validation in `ora-plugin-manager`, which is
/// the only place a plugin id is first admitted into the system.
...
impl PluginId {
    ...
    /// Returns the name segment, e.g. `ora-space.skillhub`.
...
/// Rejects empty segments and any character outside the lowercase slug alphabet plus `.`.
fn validate_segment(value: &str, segment: PluginIdSegment) -> Result<(), PluginIdError> {
    if value.is_empty() {
...
mod tests {
    ...
    fn parses_canonical_form() {
        let id = PluginId::parse("official/ora-space.skillhub").expect("valid id");
        assert_eq!(
            (
                id.namespace(),
                id.name(),
                id.canonical(),
                id.to_string(),
                PluginId::new("official", "ora-space.skillhub"),
            ),
            (
                "official",
                "ora-space.skillhub",
                "official/ora-space.skillhub".to_string(),
                "official/ora-space.skillhub".to_string(),
                Ok(id.clone()),
            )
        );
    ...
    fn rejects_malformed_ids() {
        let cases = [
            ("ora-space.skillhub", Err(PluginIdError::MissingSeparator)),
            (
                "/skillhub",
    ...
    fn serde_round_trips_canonical_string() {
        ...
        assert_eq!(
            (
                ...
                serde_json::from_str::<PluginId>("\"official/ora-space.skillhub\"").ok(),
                serde_json::from_str::<PluginId>("\"ora-space.skillhub\"").is_err(),
            ),
            (
                "\"official/ora-space.skillhub\"".to_string(),
                Some(id),
