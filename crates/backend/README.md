...
- `PluginApi` composes `ora-plugin-lifecycle`, `ora-plugin-config`, SQLite, the bundled Deno runtime, and one `AppEventHub`. It is the only owner of plugin processes and the transport-facing Plugin Configuration editor operations: `Backend::open` discovers installed packages once under `BackendPaths::data_directory`, the agent runtime reaches a plugin only by attaching through `PluginApi`, and later scans, lifecycle actions, and revision-checked configuration changes flow through the stable `Backend` API. Plugin Configuration is host-owned in this delivery and is not injected into Agent processes.
