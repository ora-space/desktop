...
- `PluginApi` composes `ora-plugin-lifecycle` with SQLite, the bundled Deno runtime, one `AppEventHub`, and a broadcast `PluginNotificationSink` whose receivers (`PluginGateway::subscribe_notifications`) let the desktop surface host route plugin-originated notifications such as `ui/push` without blocking the lifecycle pump. `Backend::open` scans plugin identity at startup without auto-activation; later scans and lifecycle actions flow through the stable `Backend` API.
