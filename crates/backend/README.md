...
Task creation and the database phase of aggregate deletion share a short synchronous lifecycle lock so a cascade cannot miss Git state being provisioned concurrently. Session creation, load admission, and that database phase also share an asynchronous lifecycle lock. Create holds it through Running persistence, and load acknowledges admission only after the same durable transition, so a cascade cannot delete the owning checkout during provider setup. SQLite and Git execute in a blocking job that continues through cleanup even if its caller is cancelled; both lifecycle locks are released immediately after the database commit, before post-commit Git cleanup begins. Session deletion remains owned by the agent runtime, and aggregate deletion does not delete provider-owned ACP history.
