...
- `PluginApi` composes `ora-plugin-lifecycle` with SQLite, the bundled Deno runtime, one `AppEventHub`, and a logging `PluginNotificationSink`. `Backend::open` scans plugin identity at startup without auto-activation; later scans and lifecycle actions flow through the stable `Backend` API.
