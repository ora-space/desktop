...
- `Backend::open` also composes `ora-plugin-lifecycle` with SQLite, the bundled Deno runtime, and one `AppEventHub`. Plugin identity is scanned at startup without auto-activation; later scans and lifecycle actions flow through the stable `Backend` API.
