...
- `PluginApi` composes `ora-plugin-lifecycle` with SQLite, the bundled Deno runtime, one `AppEventHub`, and a broadcast `PluginNotificationSink` whose receivers (`PluginGateway::subscribe_notifications`) let the desktop surface host route plugin-originated notifications such as `ora/ui/push` without blocking the lifecycle pump. `Backend::open` scans plugin identity at startup without auto-activation; later scans and lifecycle actions flow through the stable `Backend` API.
- `Backend::plugin_gateway` returns the `PluginGateway` the desktop surface layer drives: installed-package lookup, the plugin's writable data directory, on-demand process connections (`ensure_running` / `connection`), idle stop, and the `SurfaceCloser` injection point. Agent plugins launched by the agent runtime take their Deno permissions from `ora_plugin_lifecycle::agent_permissions`, the same source the lifecycle launcher uses.
