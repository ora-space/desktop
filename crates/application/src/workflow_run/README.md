...
- Soft deletion refuses active runs (a `Running` run, a non-terminal node run, or a `Running` session bound to one of the run's node runs) and then cascades a soft-delete across the run, its node runs, and those node-owned sessions in one transaction. It never deletes or retires the shared Workspace.
