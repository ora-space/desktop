...
- Worktree-mode tasks require a valid Git repository and an existing local base branch, reserve a non-colliding task id/branch prefix, resolve the selected base branch to an immutable commit id, create a linked worktree from that commit, persist its `Worktree` record, and then persist the `Task` that owns it.
...
The frontend lists local project branches before creation. Ora-managed `ora/<prefix>` branches retain their Git identity in requests but use the owning task title as their display label.
