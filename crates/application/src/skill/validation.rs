...
/// Parses the skill manifest into typed application failures without exposing parser diagnostics.
pub(crate) fn parse_skill_manifest(contents: &str) -> Result<SkillManifest, ApplicationError> {
    let matter = Matter::<YAML>::new();
    let parsed = matter.parse::<SkillManifest>(contents).map_err(|source| {
    ...
    let manifest = parsed
        .data
        .ok_or_else(|| ApplicationError::SkillManifestInvalid {
            source: Box::new(std::io::Error::other(
                "SKILL.md does not define name and description front matter",
            )),
        })?;
    ...
    if name.is_empty() {
        return Err(ApplicationError::SkillManifestNameBlank);
    }
    if description.is_empty() {
        return Err(ApplicationError::SkillManifestDescriptionBlank);
    }
...
mod tests {
    ...
    fn parses_manifest_and_reports_blank_or_missing_fields() {
        ...
        assert_eq!(
            ...
            Err(ApplicationError::SkillManifestInvalid {
                source: Box::new(std::io::Error::other(
                    "SKILL.md does not define name and description front matter"
                )),
            })
        ...
        assert_eq!(
            parse_skill_manifest("---\nname: grilling\ndescription: \"  \"\n---\n"),
            Err(ApplicationError::SkillManifestDescriptionBlank)
        );
    }
}
