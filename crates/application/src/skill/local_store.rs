...
impl SkillPackageStore for LocalSkillPackageStore {
    ...
    fn write_file(
        ...
        {
            return Err(SkillPackageStoreError::new(std::io::Error::new(
                ErrorKind::InvalidInput,
                "refusing to stage an unsafe relative path",
            )));
        }
    ...
    fn promote(&self, skill_id: &SkillId, name: &str) -> Result<(), SkillPackageStoreError> {
        ...
        if destination.exists() {
            return Err(SkillPackageStoreError::new(std::io::Error::new(
                ...
                "committed skill directory already exists",
            )));
        }
...
fn operation_failed(error: std::io::Error) -> SkillPackageStoreError {
    SkillPackageStoreError::new(error)
}
...
mod tests {
    ...
    fn promote_refuses_to_overwrite_an_existing_committed_directory() {
        ...
        let error = store
            .promote(&contender, "grilling")
            .expect_err("a committed directory must not be overwritten");
        assert_eq!(
            error.source().map(ToString::to_string),
            Some("committed skill directory already exists".to_string())
        );
    }
