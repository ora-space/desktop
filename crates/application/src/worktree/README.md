...
Repository implementations must preserve stored branch identity and soft-delete semantics. They must not reconstruct checkout paths, invoke Git, or decide when a worktree should be created or removed. Creation and compensation belong to `TaskWorktreeProvisioner`; post-cascade cleanup belongs to `TaskGitResourceCleaner`; concrete persistence belongs to `ora-db`.
