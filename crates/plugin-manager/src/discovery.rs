...
/// Discovers valid direct child packages below the installed root and isolates every
/// recoverable failure so one broken package never hides its siblings.
///
/// The directory name is not part of a package's identity: the manifest alone names the plugin,
/// and two directories claiming the same id are reported rather than silently merged.
...
/// Returns real direct child directories of the installed root in reproducible path order, or
/// `None` when the root cannot be listed at all. A missing root is an empty installation.
