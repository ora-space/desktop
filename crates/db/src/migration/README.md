...
- The default catalog contains seven dependency-ordered modules: workspace core and application configuration, Agent/Skill catalog, workflows, Git lifecycle bookkeeping, Workspace Effect state, durable plugin marketplace source configuration, and the immutable marketplace-source namespace bindings plus per-source marketplace enabled flag.
...
- Migration `0007` records the namespace bound to each marketplace source's canonical URL and adds
  an `enabled` flag. The namespace is a table of its own rather than a column on the source row
  because the binding outlives the configuration: once a plugin from that source is installed, the
  namespace is frozen into its install path, private data directory, `skills` rows, and Effect
  Consumer identity, so deleting the source must leave the binding behind and re-adding the
  repository must reuse it. Nothing updates or deletes a binding row — writing a second identity
  for one repository would strand every row the first identity owns. Disabling a source keeps its
  URL, branch, proxy policy, position, and namespace binding, but drops it from marketplace sync,
