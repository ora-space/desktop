use ora_application::{
    RepositoryError, SkillImportCommitError, SkillImportUnitOfWork, SkillPackageStoreError,
};
...
impl SkillImportUnitOfWork for SqliteSkillImportUnitOfWork {
        ...
        OnInserted: FnOnce() -> Result<(), SkillPackageStoreError>,
    {
        ...
        let outcome = self.pool.with_connection_mut(|connection| {
            ...
            if let Err(source) = insert_skill_row(&transaction, &skill) {
            ...
            match on_inserted() {
                Ok(()) => match transaction.commit() {
                    Ok(()) => Ok(Ok(())),
                    Err(source) => Ok(Err(SkillImportCommitError::CommitAfterPromote {
                        source: RepositoryError::new(source),
                    })),
                },
                Err(source) => {
                    ...
                    drop(transaction);
                    Ok(Err(SkillImportCommitError::Promote { source }))
                }
            }
        });
        ...
        match outcome {
            Ok(inner) => inner,
            Err(source) => Err(SkillImportCommitError::Insert {
                source: RepositoryError::new(source),
            }),
        }
    }
