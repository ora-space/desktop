...
use ora_effect::{
    ...
    EffectTargetId, Generation, LocalTimestamp, ManagedIdentity, ManagedItem, OperationArtifact,
...
pub(super) fn save_current_state(
        ...
        transaction.execute(
            "INSERT INTO effect_managed_items (
             ...
             ON CONFLICT(id) DO UPDATE SET
                 ...
                 applied_generation = excluded.applied_generation, updated_at = MAX(effect_managed_items.updated_at, excluded.updated_at)",
            params![
    ...
    for status in target_statuses {
        save_target_status(transaction, status, updated_at)?;
    }
    for status in resource_statuses {
        save_resource_status(transaction, status, updated_at)?;
    }
