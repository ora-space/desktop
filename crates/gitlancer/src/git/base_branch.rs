...
/// Parses refs into one logical branch map, allowing a fetched remote ref to replace a stale local duplicate.
fn parse_worktree_bases(stdout: &str, remote_name: Option<&str>) -> Vec<WorktreeBase> {
        ...
        .filter(|line| !line.is_empty())
    {
        ...
        if let (Some(remote_name), Some(remote_prefix)) = (remote_name, remote_prefix.as_deref())
            ...
            && branch_name != "HEAD"
        {
            // A fetched remote-tracking ref is authoritative for new worktree bases.
            bases.insert(
