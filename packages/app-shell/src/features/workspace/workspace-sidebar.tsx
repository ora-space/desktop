                              ...
                              icon={conversations[session.id]?.pendingPermissions.length
                                ...
                                : conversations[session.id]?.isResponding
                                  ...
                                  : unread.has(session.id)
                                    ...
                                    : null}
                              label={conversations[session.id]?.sessionTitle ?? session.agentCli}
                              onClick={() => selectSession(session.id, task.id, project.id)}
                              menu={(
                                <EntityMenu
                                  onDelete={() => setDeleteTarget({ kind: "session", id: session.id, name: conversations[session.id]?.sessionTitle ?? session.agentCli })}
                                />
                              )}
                            />
                          ))}
