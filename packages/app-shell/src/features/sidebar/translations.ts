// Pure translation data: safe to compose without importing feature implementation.
export const sidebarTranslations = {
  "zh-CN": {
    "sidebar.collapse": "收起侧边栏",
    "sidebar.expand": "展开侧边栏",
    "sidebar.back": "返回",
    "sidebar.resize": "调整侧边栏宽度；双击恢复默认宽度",
    "sidebar.search": "搜索工作区",
    "sidebar.clearSearch": "清除搜索",
    "sidebar.newProject": "新建项目",
    "sidebar.workspace": "工作区",
    "sidebar.projectCount": "{{count}} 个项目",
    "sidebar.loading": "正在加载工作区…",
    "sidebar.empty": "未找到项目。",
    "sidebar.newSession": "新建会话",
    "sidebar.dismissDraft": "关闭草稿",
    "sidebar.newTask": "新建工作树",
    "sidebar.newDirectChat": "新建任务",
    "sidebar.createInProject": "在此项目中新建",
    "sidebar.createInTask": "在此任务中新建",
    "sidebar.newWorkflow": "运行工作流",
    "sidebar.workflows": "工作流",
    "sidebar.searchWorkflows": "搜索工作流模板",
    "sidebar.noWorkflows": "暂无已发布的工作流。",
    "sidebar.rename": "重命名",
    "sidebar.renameTooLong": "标题最多 255 个字符。",
    "sidebar.archive": "归档",
    "sidebar.archiveSoon": "归档功能即将推出，可右键该行并选择「删除」",
    "sidebar.console": "控制台",
    "sidebar.openActions": "打开操作菜单",
    "sidebar.navigation": "主导航",
    "sidebar.projects": "项目",
    "sidebar.permissionRequired": "等待权限确认",
    "sidebar.unread": "有未读更新",
    "sidebar.directChatTask": "直聊任务",
    "sidebar.worktreeTask": "Git 工作树任务",
    "sidebar.session": "会话",
    "sidebar.sectionWorkflows": "工作流",
    "sidebar.sectionSessions": "会话",
    "sidebar.sectionWorktrees": "工作树",
    "sidebar.emptyWorkflows": "暂无工作流",
    "sidebar.emptyWorkflowsFiltered": "暂无该状态的工作流",
    "sidebar.emptySessions": "暂无会话",
    "sidebar.emptyWorktrees": "暂无工作树",
    "sidebar.filterWorkflowStatus": "筛选工作流状态",
    "sidebar.filterWorkflowStatusAll": "全部状态",
    "sidebar.startWorkflow": "启动已挂载工作流",
    "sidebar.workflowRun": "工作流运行",
    "account.label": "{{name}} 的账户",
    "account.unknownIdentity": "匿名用户",
    "account.language": "语言",
    "account.switchEnglish": "English",
    "account.switchChinese": "简体中文",
  },
  "en-US": {
    "sidebar.collapse": "Collapse sidebar",
    "sidebar.expand": "Expand sidebar",
    "sidebar.back": "Back",
    "sidebar.resize": "Resize sidebar; double-click to reset its width",
    "sidebar.search": "Search workspace",
    "sidebar.clearSearch": "Clear search",
    "sidebar.newProject": "New project",
    "sidebar.workspace": "Workspace",
    "sidebar.projectCount": "{{count}} projects",
    "sidebar.loading": "Loading workspace...",
    "sidebar.empty": "No projects found.",
    "sidebar.newSession": "New session",
    "sidebar.dismissDraft": "Dismiss draft",
    "sidebar.newTask": "New worktree",
    "sidebar.newDirectChat": "New task",
    "sidebar.createInProject": "Create in this project",
    "sidebar.createInTask": "Create in this task",
    "sidebar.newWorkflow": "Run workflow",
    "sidebar.workflows": "Workflows",
    "sidebar.searchWorkflows": "Search workflow templates",
    "sidebar.noWorkflows": "No published workflows yet.",
    "sidebar.rename": "Rename",
    "sidebar.renameTooLong": "Title can be at most 255 characters.",
    "sidebar.archive": "Archive",
    "sidebar.archiveSoon":
      "Archive is coming soon. Right-click the row and choose Delete.",
    "sidebar.console": "Console",
    "sidebar.openActions": "Open actions",
    "sidebar.navigation": "Main navigation",
    "sidebar.projects": "Projects",
    "sidebar.permissionRequired": "Permission required",
    "sidebar.unread": "Unread updates",
    "sidebar.directChatTask": "Direct chat task",
    "sidebar.worktreeTask": "Git worktree task",
    "sidebar.session": "Session",
    "sidebar.sectionWorkflows": "Workflows",
    "sidebar.sectionSessions": "Sessions",
    "sidebar.sectionWorktrees": "Worktrees",
    "sidebar.emptyWorkflows": "No workflows",
    "sidebar.emptyWorkflowsFiltered": "No workflows with this status",
    "sidebar.emptySessions": "No sessions",
    "sidebar.emptyWorktrees": "No worktrees",
    "sidebar.filterWorkflowStatus": "Filter workflow status",
    "sidebar.filterWorkflowStatusAll": "All statuses",
    "sidebar.startWorkflow": "Start mounted workflow",
    "sidebar.workflowRun": "Workflow run",
    "account.label": "{{name}} account",
    "account.unknownIdentity": "Anonymous user",
    "account.language": "Language",
    "account.switchEnglish": "English",
    "account.switchChinese": "简体中文",
  },
} as const;
