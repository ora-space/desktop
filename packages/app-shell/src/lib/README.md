...
`stripTaskCwdPrefix` before `readWorkspaceFile` or Diff requests. OS handoff
uses `joinOsAbsolutePath` so Explorer, VS Code, and Copy path receive a host
path rather than an internal URI.
