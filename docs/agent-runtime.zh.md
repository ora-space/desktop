...
已配置的 MCP 插件属于 Session Runtime Input。`startSession`、prompt attach、重建、在线刷新、workflow 启动和 Agent 替换都会解析一个 Snapshot，并作为 ACP `mcpServers` 发送。在线 Session 在内存中维护 Desired/Active revision；配置变化立即唤醒空闲 Session，并在繁忙 Session 的当前 prompt 结束后刷新。MCP 刷新与 Skill Effect 修改共享同一个 Agent Session Barrier。设置值不会进入 Effect 状态、SQLite、Workspace 文件、日志或 UI。设置成功不表示这些 Server 已经完成握手；详见 [Session MCP](session-mcp.zh.md#运行健康)。
