...
- The response carries the new CLI's `availableCommands` and `configOptions`. ACP reports both only while a session is being created or loaded, and the switch performs the only handshake this session will get, so anything not returned here can never be asked for again — a client that heard only about the rebound session would keep offering the previous CLI's models.
