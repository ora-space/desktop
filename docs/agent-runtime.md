...
- Load reads Ora's own record and touches no agent. It creates no provider session, registers no actor, and moves no lifecycle state, so a conversation opens whether or not the agent that produced it is still installed or startable; what the client receives is that record, including typed notices for known holes rather than a transcript that silently appears continuous. A session with no history file yet opens as an empty conversation rather than a failure. If an actor already exists it answers its own load, because only it knows the durable cutoff and the records of a turn still in flight: with a prompt in progress it replays the record through that attachment point and then follows later events, neither starting a second prompt nor taking ownership of the first. See [Session History](#session-history).
- A prompt is what acquires a provider session. A session holding no channel attaches first: it registers a route on the current connection generation and calls `session/load` with the private `agentSessionId`, drains and discards whatever the agent recites, and marks the row Running. Configuration options and available commands reported by that handshake reach the client as session updates ahead of the turn, and are not recorded — they describe the provider serving the conversation, not what was said in it. Admission waits on the attach, so a send that cannot reach an agent fails as a send rather than as a turn that produced nothing. Lifecycle status is not a precondition of prompting; a session that has only been read attaches on its first message.
- An agent that does not support `session/load`, or that fails to restore the session, does not end the conversation: Ora builds a replacement provider session with `session/new` and the next prompt carries the recorded transcript into it, exactly as an agent switch does. Nothing is written to history for this — the rebuilt binding stays out of the row until the prompt carrying the transcript is accepted, so a crash in between leaves the old binding in place and the following prompt rebuilds and re-delivers rather than continuing against an agent that holds nothing.
