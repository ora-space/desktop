...
- Whether the current binding still needs the transcript is derived from the record — a trailing `AgentSwitched` with no user message after it — so it survives a restart without any stored flag.
