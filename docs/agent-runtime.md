...
- Create calls `session/new` on the ready shared connection and persists the Ora Session only after setup succeeds. A shared admission lock prevents Task or Project deletion from entering its cascade while that handshake is in flight, while the guarded insert remains a final ownership check.
...
Session deletion serializes against new actor operations, unloads its route, and then soft-deletes the row under the same lifecycle guard. Task and Project deletion share that guard with create and load admission, reject Running descendants, and transactionally cascade stopped Ora records in a blocking job. Project deletion first waits for the owning Project's task-provisioning permit without holding the global Session guard. Lifecycle guards are released immediately after commit. The same job then delegates each target to `TaskGitResourceCleaner`, so caller cancellation cannot skip cleanup after a commit; failures are logged without changing the deletion response and are not retried durably. Aggregate deletion does not call ACP session delete or remove provider-owned history.
