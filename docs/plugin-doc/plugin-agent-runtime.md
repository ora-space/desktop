...
单个 agent 的状态机（复用现有 `ConnectionState` 三态，语义对齐）：
...
**熔断**沿用 `plugin-runtime.md` 已定的策略：1 分钟内崩溃超过 3 次标记 `Failing`，停止自动
重启并在 UI 提示。这一层归 `PluginRuntime` 的 supervisor，agent 侧只观察到连接持续
`Unavailable`。
...
- **协议层**（`plugin-runtime`）：`duplex` 管道 + 手写帧，覆盖 `emits` 白名单、未声明方法杀
  进程、`notify` 不占用 pending 表、插件通知与响应交错时的相关性。
