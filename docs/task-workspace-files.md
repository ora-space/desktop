...
- `packages/app-shell/src/features/files` owns the file tree, viewer, search UI,
  ...
  shows the file, and selects the optional line. A hit outside the task cwd is
  not opened as a worktree-relative path. Missing files show the localized
  missing-path copy rather than the raw transport error. The backend still
