...
- `UpdateTaskRequest` cannot change project ownership. Task creation resolves the requested project's Git root. Task and Project deletion keep their existing public responses while the backend force-removes task-owned linked worktrees and local branches after the database cascade commits. Git cleanup is best effort and is not represented as a partial-success contract state.
