...
- Task creation resolves the requested project's Git root at creation time. After a successful Task or Project cascade, the backend invokes `TaskGitResourceCleaner` outside the lifecycle locks. SQLite and Git work run in a blocking job that continues through cleanup if the caller is cancelled.
