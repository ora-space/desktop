...
- `UpdateTaskRequest` cannot change project ownership. Task creation resolves the requested project's Git root. Task and Project deletion keep their existing public responses while the backend invokes `TaskGitResourceCleaner` after the database cascade commits. Cleanup resolves by validated branch first, uses the exact deterministic task path only as a detached fallback, refuses the main checkout, and deletes only the validated Ora branch. Git cleanup is best effort and is not represented as a partial-success contract state.
