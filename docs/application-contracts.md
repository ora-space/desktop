...
- Worktree task creation accepts a selected base ref. The backend `BranchLister` adapter fetches `upstream` or falls back to `origin`, prefers refreshed remote refs over stale local duplicates, and the application-owned `ListProjectBranchesHandler` maps managed Ora branches to task titles. Creation resolves the selected ref to a commit ID before mutation.
