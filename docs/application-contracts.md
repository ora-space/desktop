...
- Project and Task deletion soft-delete the complete Ora-owned aggregate in one SQLite transaction. A Running Session rejects the operation with `resource_in_use`; deletion waits for an in-flight create or load admission to expose its durable Running row before continuing. Stopped children are cascaded. The SQLite transaction never calls Git, and aggregate deletion never deletes provider-owned ACP history.
- Task creation resolves the requested project's Git root in a blocking worker under a shared per-Project lifecycle permit. Concurrent creations and unrelated aggregate operations remain independent; deleting the owning Project takes the exclusive permit before entering its cascade. After a successful Task or Project cascade, the backend invokes `TaskGitResourceCleaner` outside the lifecycle guards. The blocking deletion job continues through cleanup if the caller is cancelled.
- Cleanup requires a full UUID task id, resolves by validated branch first, uses the exact deterministic task path only as a detached fallback, refuses the main checkout, and deletes only the validated Ora branch. Missing resources and failures are logged; cleanup remains best effort and is not represented as a partial-success contract state.
