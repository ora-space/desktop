...
- `ora-contracts` owns serialization-friendly request, response, and stream-event DTOs for Project, Task, Session, Skill, Agent, and Git identity operations, including structured ACP prompts and provider-advertised commands, plus the Web-only project work context and filesystem operations.
