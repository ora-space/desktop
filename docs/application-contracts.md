...
- Worktree task creation accepts a selected base ref. The backend fetches `upstream` or falls back to `origin`, prefers refreshed remote refs over stale local duplicates, and resolves the selected ref to a commit ID before mutation. The project branch-list contract supplies the logical branch name, resolvable ref name, and a display name that replaces managed Ora branch identifiers with task titles.
