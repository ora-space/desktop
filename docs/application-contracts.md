...
- Project and Task deletion use one SQLite transaction to soft-delete the complete Ora-owned aggregate. A Running Session rejects the operation with `resource_in_use`; deletion waits for an in-flight create or load admission to finish, and every successful admission exposes its Running row before deletion can continue. Stopped children are cascaded. The SQLite cascade never calls Git and aggregate deletion never deletes provider-owned ACP history.
...
- `UpdateTaskRequest` cannot change project ownership. Task creation resolves the requested project's Git root. Task and Project deletion keep their existing public responses while the backend invokes `TaskGitResourceCleaner` after the database cascade commits and lifecycle locks have been released. SQLite and Git work run in a blocking job that continues through cleanup if the caller is cancelled. Cleanup requires a full UUID task id, resolves by validated branch first, uses the exact deterministic task path only as a detached fallback, refuses the main checkout, and deletes only the validated Ora branch. Missing resources and failures are logged; Git cleanup remains best effort and is not represented as a partial-success contract state.
- Worktree paths are composed when creating a new worktree. Session execution resolves existing worktrees from persisted branch identity; aggregate cleanup may reconstruct `configured_worktree_root/full_task_id` only for an exact-root fallback.
