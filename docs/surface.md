...
The `surface_*` commands are listed in `app_commands.rs` and `permissions/app-commands.toml`,
granted to the `main` webview by `capabilities/default.json`. `remote-surface:*` webviews are
named by no capability and are denied every app, plugin, and core command
(`tests/command_acl.rs`, `task lint:acl`).
