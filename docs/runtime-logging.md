...
- Plugin processes' own diagnostics are not runtime log events: the host persists them per plugin into a separate JSONL file that reuses this event envelope. Only host facts about a plugin, including failures of that plugin-log pipeline, are runtime log events. See [Plugin Logging](plugin-logging.md).
