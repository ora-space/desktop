...
- Runtime composition roots own reading configuration, calling `ora_logging::init_logging` with an explicit `LoggingConfig`, retaining the returned `LoggingGuard` for the rest of the process lifetime, and exposing its cloneable health handle through backend diagnostics when needed. The guard keeps the non-blocking file writer alive; dropping it early loses buffered output.
...
`ORA_LOG_MODE=stdout` writes JSON lines to standard output only — no files are created and retention cleanup does not run. File-backed modes rotate when the non-blocking worker processes its first write after midnight in the configured process timezone. The filename suffix is the worker's local processing date. An event formatted before midnight but processed after midnight can therefore appear in the next day's file while retaining its earlier JSON timestamp. Startup cleans the oldest matching files synchronously. Each successful rollover schedules coalesced cleanup on a dedicated retention worker so directory scans and deletions do not block the non-blocking writer. `stdout_and_file` emits every event to both sinks using the same envelope.

File output intentionally keeps the lossy `tracing_appender` queue so logging backpressure cannot stall application threads. `LoggingGuard::health()` returns a `LoggingHealthSnapshot`, and `LoggingGuard::health_handle()` returns a cloneable handle suitable for backend health services. The snapshot reports active rotation, output-write, output-flush, and retention issues together with cumulative failure counters and the number of events discarded after the queue reached capacity.
...
The RFC 3339 timestamp records the event formatting time in the configured process timezone and includes its UTC offset. Daily log filename dates and rollover boundaries use that same timezone but are selected later from the non-blocking worker's processing time. Files are therefore processing-date buckets rather than strict partitions of event timestamp dates. If the next file cannot be opened, logging continues in the previous file and retries with exponential backoff from one second up to sixty seconds. Failure to create the log directory or open the initial file rejects initialization. Failure to delete old files does not reject initialization or stop file writes; it appears as an active retention issue until a later cleanup succeeds.
