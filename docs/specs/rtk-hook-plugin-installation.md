...
The first milestone must establish a trustworthy installation loop without taking ownership of Agent-specific configuration. A user must be able to discover the RTK Hook Plugin, download the correct Windows x86_64 artifact, verify and install it, see it enabled, disable it, and uninstall it. The installed package must contain a real, runnable RTK binary, while installation itself must never execute downloaded code.
...
Install Hook Plugins with static validation only. After installation, attempt the existing automatic enable behavior. If another Enabled Hook owns the same command alias, retain the installation but leave the new Hook disabled and return a typed conflict outcome. Establish payload runnability in the packaging workflow and isolated end-to-end tests, not inside the installer.
...
10. As an Ora user, I want RTK automatically enabled after a successful conflict-free installation, so that the plugin follows the current Ora installation behavior.
...
12. As an Ora user, I want a conflicting Hook to remain installed but disabled, so that I can inspect it and resolve the conflict deliberately.
13. As an Ora user, I want disabling RTK to preserve the installed package, so that I can re-enable it without downloading it again.
14. As an Ora user, I want uninstalling RTK to remove its installed package and global eligibility state, so that Ora no longer treats it as available locally.
...
20. As a plugin author, I want one Hook Plugin to contribute exactly one Hook, so that identity, versioning, enablement, and errors remain unambiguous.
...
22. As a plugin author, I want target-specific artifacts to share one canonical Plugin identity, so that adding platforms does not fragment marketplace presence or enablement.
...
27. As an Ora maintainer, I want Hook Plugins to be processless, so that installing or enabling one does not create an unnecessary Deno runtime.
...
31. As an Ora maintainer, I want Hook command aliases normalized and collision-checked across Enabled Hooks, so that execution resolution is deterministic.
32. As an Ora maintainer, I want installation and activation outcomes modeled as a closed enum, so that callers cannot observe contradictory success flags.
...
- Use canonical Rust target triples. The first RTK artifact supports only `x86_64-pc-windows-msvc`; target matching never falls back across architecture, operating system, libc, or ABI.
...
- Validate a normalized bare command alias that cannot contain path separators. Enabled Hook command aliases must be unique.
- Preserve automatic enable after install. Model the result as either installed-and-enabled or installed-but-disabled with a typed reason and conflicting Plugin identity.
- Do not execute the Hook payload during marketplace sync, download, import, discovery, installation, enablement, disablement, or uninstall.
...
- Use the repository glossary terms Hook Plugin, Hook Configuration, Hook Protocol, Hook Command, Hook Target, Targeted Artifact, Enabled Hook Plugin, and Runnable Hook Package.

## Testing Decisions

- Prefer one high-level backend installation seam that exercises registry manifest parsing, target selection, injected download, digest verification, archive extraction, installed-manifest parsing, Hook Configuration compilation, filesystem containment, discovery, automatic enablement, command conflict handling, disablement, and uninstall against a temporary Ora data directory.
...
- Assert external outcomes: installed files and identity, typed installation outcome, enabled state, incompatibility reason, conflict identity, and final uninstall state. Avoid tests that merely mirror internal helper calls or private fields.
...
- Add backend tests for both installation outcomes: installed-and-enabled and installed-but-disabled because of a command conflict.
...
- Complete the milestone with a real clean-data-directory E2E using a compatible Windows x86_64 Ora build and the marketplace PR branch, covering sync, compatible display, download, SHA verification, install, automatic enable, disable, and uninstall.
...
- Workspace or project-level Hook selection; enablement remains application-global.
- Injecting Hook directories into real Agent process PATH values.
- Executing RTK as part of plugin installation or enablement.
...
- Consumer reconciliation for Hook disable, uninstall, or upgrade. Before Hook consumption ships, a later design must require affected Agents to become idle and restart before an in-use payload is removed.
...
- Existing processless plugins report an enabled/stopped lifecycle state. Changing the general runtime-status model for processless contributions is not required for this milestone.
