...
| `0007`  | Immutable marketplace-source namespace bindings keyed by canonical Git URL, plus a per-source `enabled` flag so a marketplace URL can be disabled without deleting its identity.                               |
