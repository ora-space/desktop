...
Marketplace metadata still comes from Git-synced registry sources. A listing's `url` may be a public HTTPS release URL (downloaded unsigned), a bare object-store key, or a path-style HICS URL such as `https://s3-hc-dgg.hics.huawei.com/ora-marketplace-1.0/agent/pkg.orax` (object key `agent/pkg.orax`). Keys and matching path-style bucket URLs are fetched with a SigV4 path-style GET against the Huawei HICS marketplace bucket. SHA-256 verification and extraction are unchanged.
