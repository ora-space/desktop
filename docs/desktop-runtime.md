...
The configured root is primarily a creation target. Agent Sessions resolve existing worktree locations from the stored branch name and `git worktree list --porcelain`. Task and Project deletion first commits the database cascade, then best-effort resolves each checkout by its validated branch; if the branch association is absent, cleanup accepts only an exact `configured_worktree_root/full_task_id` match and explicitly refuses the main checkout. A later creation-root change can therefore limit detached-checkout fallback, while ordinary branch-associated cleanup still resolves from Git metadata.
