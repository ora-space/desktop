...
Marketplace metadata still comes from Git-synced registry sources. A listing's `url` is either an HTTPS release URL, downloaded unsigned, or an object-store key, fetched with a SigV4 path-style GET against the Huawei HICS marketplace bucket. SHA-256 verification and extraction are unchanged.
