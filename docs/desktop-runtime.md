...
Desktop constructs one cloneable `ora-backend::Backend`. Unary operations use typed snake-case Tauri commands. Session load and structured ACP prompt operations use `stream_contract`, which forwards ordered `data`, `error`, and `end` frames over a Tauri Channel. A private call id allows an `AbortSignal` to cancel only that stream.
