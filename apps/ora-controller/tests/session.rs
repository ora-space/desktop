...
/// With nothing to query, every tick still sends one uplink frame so the Node's per-frame read
/// deadline keeps meaning Controller liveness instead of expiring on an idle session.
#[test]
fn idle_session_sends_one_heartbeat_per_query_tick() {
    ora_logging::with_trace_logging(|| {
        let root = tempfile::Builder::new()
            .permissions(fs::Permissions::from_mode(/*mode*/ 0o700))
            .tempdir_in(std::env::var_os("HOME").unwrap())
        ...
        tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap().block_on(async {
            ...
            let peer = async {
                ...
                let identity = NodeRuntimeIdentity { node_id: endpoint.node_id.clone(), incarnation_id: NodeIncarnationId::new("current") };
                write_node_message(&mut stream, &NodeToControllerMessage::HelloAccepted(HelloAcceptedMessage { protocol_version: CURRENT_PROTOCOL_VERSION, payload: HelloAccepted { selected_version: CURRENT_PROTOCOL_VERSION, node: identity.clone(), capabilities: vec![NodeCapability::RepositoryClone] } })).await.unwrap();
                let expected = ControllerToNodeMessage::Heartbeat(ControllerHeartbeatMessage { protocol_version: CURRENT_PROTOCOL_VERSION, payload: ControllerHeartbeat { controller_id: ControllerId::new("owner") } });
                for _ in 0..5 {
                    let message = timeout(Duration::from_millis(/*millis*/ 500), read_controller_message(&mut stream)).await.unwrap().unwrap().unwrap();
                    assert_eq!(message, expected);
                    // Keep the Controller's own liveness deadline satisfied, as a real Node would.
                    write_node_message(&mut stream, &NodeToControllerMessage::Heartbeat(HeartbeatMessage { protocol_version: CURRENT_PROTOCOL_VERSION, payload: Heartbeat { node: identity.clone() } })).await.unwrap();
                }
            };
            tokio::select! { _ = session => panic!("idle session ended before peer checks"), _ = peer => {} }
        });
    });
}
