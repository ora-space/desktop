        ...
        while let Some(effect) = queue.pop_front() {
            match effect {
                ...
                SurfaceEffect::CreateWebview { target, operation } => {
                    ...
                    if created
                        && let Some(record) = self.registry.record(instance)
                        && let Some(start) = announce_opened(&self.gateway, &self.registry, &record)
                    {
                        let gateway = self.gateway.clone();
                        tauri::async_runtime::spawn(async move { start.run(&gateway).await });
                    }
                }
