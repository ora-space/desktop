// Application commands exposed to the `main` webview through Tauri IPC.
//
// This file is shared by `build.rs` (so tauri-build autogenerates `allow-<command>` /
// `deny-<command>` permissions for the app ACL manifest) and by `src/lib.rs` (whose
// `acl_` test asserts it matches the `generate_handler!` registration list). Keep it
// in sync with `generate_handler!`; `task lint:acl` fails on drift.
pub const APP_COMMANDS: &[&str] = &[
    "create_project",
    "get_project",
    "list_projects",
    "list_project_branches",
    "update_project",
    "delete_project",
    "create_task",
    "get_task",
    "list_tasks",
    "update_task",
    "delete_task",
    "get_task_workspace",
    "get_task_diff",
    "commit_task_changes",
    "push_task_branch",
    "list_task_diff_comments",
    "create_task_diff_comment",
    "reply_task_diff_comment",
    "set_task_diff_comment_status",
    "list_workspace_directory",
    "read_workspace_file",
    "search_workspace",
    "get_spec_catalog",
    "read_spec",
    "warm_session",
    "set_session_config",
    "attach_session",
    "get_session",
    "list_sessions",
    "respond_to_session_permission",
    "stop_session",
    "switch_session_agent",
    "resume_session_history",
    "delete_session",
    "rename_session",
    "stream_contract",
    "cancel_contract_stream",
    "get_agent_runtime_status",
    "list_agent_models",
    "create_skill",
    "get_skill",
    "list_skills",
    "update_skill",
    "delete_skill",
    "prepare_skill_import",
    "get_skill_import",
    "commit_skill_import",
    "cancel_skill_import",
    "create_agent",
    "get_agent",
    "list_agents",
    "list_installed_plugins",
    "scan_plugins",
    "enable_plugin",
    "disable_plugin",
    "activate_plugin",
    "stop_plugin",
    "uninstall_plugin",
    "update_agent",
    "delete_agent",
    "prepare_agent_import",
    "commit_agent_import",
    "get_git_identity",
    "create_workflow",
    "get_workflow",
    "list_workflows",
    "update_workflow",
    "delete_workflow",
    "get_workflow_draft",
    "update_workflow_draft",
    "publish_workflow",
    "rollback_workflow",
    "activate_workflow",
    "list_workflow_versions",
    "get_workflow_version",
    "delete_workflow_snapshot",
    "get_workflow_snapshot",
    "create_workflow_run",
    "get_workflow_run",
    "list_workflow_runs",
    "list_workflow_runs_by_workflow",
    "list_workflow_node_runs",
    "delete_workflow_run",
    "start_workflow_run",
    "cancel_workflow_run",
    "restart_workflow_run",
    "update_workflow_run_input",
    "get_desktop_config",
    "get_developer_mode",
    "set_developer_mode",
    "get_runtime_log_level",
    "set_runtime_log_level",
    "set_worktree_root",
    "resolve_task_cwd",
    "open_location",
    "write_workflow_export",
    "get_dashboard_url",
    "get_dashboard_compare_url",
    "surface_capabilities",
    "surface_list",
    "surface_open",
    "surface_close",
    "surface_set_bounds",
    "surface_set_visible",
    "surface_popout",
    "surface_dock",
    "surface_reload",
];
