...
registry but not to that file is callable by nobody. Tauri generates one `allow-<command>` /
`deny-<command>` permission per registered command under `permissions/autogenerated/` and the
capability schemas under `gen/schemas/` at build time; both are git-ignored (`permissions/.gitignore`,
`.gitignore`) because they are derived from the registry and regenerate on every build.
